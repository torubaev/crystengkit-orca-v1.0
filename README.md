# CrystEngKit: From CIF files to ESP, NCI, and QTAIM plots in a few clicks — a practical suite for supramolecular chemistry and crystal engineering.
...
3. Choose the calculation setup: functional, basis set, dispersion correction solvent (or gas phase - if no solvent was chosen or entered), charge, and multiplicity.
...
ESP, NCI, and QTAIM workflows require real wavefunction files such as `.wfn` or `.wfx`. In CrystEngKit they are automatically generated by default, after ORCA job is done. 
...
6. Run ORCA and review the final summary for the calculated energies (uncorrected, BSSE. CP-corrected) and Experimental part.
